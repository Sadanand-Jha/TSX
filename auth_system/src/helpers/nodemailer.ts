...
const sendEmail = async (email: string) => {
  try {
    ...
    const info = await transporter.sendMail({
      ...
      text: "Camvo",
      html: `<p>Hello there 👋,</p><p>We're excited to have you join us at <strong>Camvo</strong>! To get started, please verify your email address by clicking the link below:</p><div><a href="http://localhost:3000/verification_page?token=${token}" style="display: inline-block; background-color: #4F6EE5; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Verify My Email</a></div><p>If you didn't sign up for Camvo or believe this was a mistake, you can safely ignore this email — no action is needed.</p><p>Thanks for choosing us!</p><b>— The Camvo Team</b>`
    });


    const email_verify = await EmailVerify.findOneAndUpdate({email}, {token})
